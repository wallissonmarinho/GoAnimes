...
**Jaeger / OpenTelemetry** (repo `www`, `deploy/k8s/jaeger/`): a app exporta **traces** por **OTLP HTTP** quando defines endpoint no ambiente **prd** — **Variables** (não é obrigatório marcar como Secret: é URL de Service interno do cluster). Nomes exatos que o `oracle-deploy` já grava no Secret `goanimes-env`: **`OTEL_SERVICE_NAME`**, **`OTEL_SDK_DISABLED`**, **`OTEL_EXPORTER_OTLP_ENDPOINT`**, **`OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`**. Preenche **apenas um** de `OTEL_EXPORTER_OTLP_ENDPOINT` *ou* `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` (se ambos, o SDK prioriza o endpoint de traces). Exemplo in-cluster, Pod `goanimes` → Service Jaeger: `OTEL_EXPORTER_OTLP_ENDPOINT=http://jaeger.observability.svc.cluster.local:4318` (o cliente HTTP anexa `/v1/traces`) **ou** `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT=http://jaeger.observability.svc.cluster.local:4318/v1/traces`. `OTEL_SERVICE_NAME=goanimes` e `OTEL_SDK_DISABLED` vazio (ou `false`); `OTEL_SDK_DISABLED=true` desliga o export. Se o Jaeger ainda não estiver no cluster, deixa as OTEL vazias — a app corre sem tracing.
...
O job **deploy** usa **`environment: prd`**. **Secret** obrigatório no repositório: **`OCI_SSH_PRIVATE_KEY`**. **`OCI_VM_HOST`** e **`OCI_VM_USER`** podem ser **Repository ou Environment Variables** (visíveis na UI); se não existirem, o workflow usa os **Secrets** com o mesmo nome (migração). No ambiente **`prd`**, a config da app vem de **Variables** (`vars`) e é gravada em **`deploy/oracle/.env.goanimes.deploy`** + Secret **`goanimes-env`**. Opcional **`GOANIMES_ENV_B64`** como Variable: base64 de linhas extra (ex. `base64 -i snippet.env | tr -d '\n'`) acrescentadas ao fim do ficheiro. Outras chaves que cries só no GitHub e não estiverem no `oracle-deploy.yml` não vão para o Pod até acrescentares `write_kv`/`env` lá ou as incluíres nesse base64. **`ACME_EMAIL`** (legado / Traefik conforme configurares na VM) pode continuar no **`.env`** na VM.
