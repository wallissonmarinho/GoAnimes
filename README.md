...
**Catálogo relacional:** migração `00002` cria `catalog_series` e `catalog_item` (FK `series_id`). Cada `SaveCatalogSnapshot` atualiza **numa transação** o `items_json` (itens + enrichment + fingerprints) e substitui as linhas normalizadas, para não ficar BD inconsistente se falhar a meio. Na primeira leitura após migrar, se as tabelas estiverem vazias mas o JSON tiver itens, o servidor faz backfill automático para as tabelas. Com dados nas tabelas, o carregamento usa **itens e séries** a partir do SQL (metadados de enrichment / RSS continuam no JSON).
